...
function xhijab_track_views($post_id) {
    if(is_single() && !is_admin()) {
        xhijab_set_post_views($post_id);
    }
}
add_action('wp_head', function() {
    if(is_single()) {
        global $post;
        if($post) xhijab_track_views($post->ID);
    }
});
...
add_action('wp_ajax_xhijab_like', 'xhijab_like_ajax');
add_action('wp_ajax_nopriv_xhijab_like', 'xhijab_like_ajax');
function xhijab_like_ajax() {
